...
const deleteEstadisticaEquipo = async (req, res) => {
    const inscripcionId = req.params.id; 
    ...
    try {
        ...
        const affectedRows = await estadisticasEquipoModel.deleteEstadisticaEquipo(inscripcionId);

       
        if (affectedRows === 0) {
           
            return res.status(404).json({ message: `No se encontró  la estadistica de equipo con ID ${inscripcionId}` });
        }

       
        res.status(200).json({ 
            message: `estadistica con ID ${inscripcionId} eliminado exitosamente`,
            inscripcionId: inscripcionId ,
        ...
        res.status(500).json({
            message: 'Error al intentar eliminar la estadistica',
            error: error.message,
